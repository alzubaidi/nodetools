...
exports['test nodetools mime-type'] = function(assert) {
    var tools = require('../lib/tools');

    assert.equal('application/json', tools.detectType('json'), 'Test .json string = application/json');
    assert.equal('application/json', tools.detectType(__dirname + '/../package.json'), 'Test .json file = application/json');
}
